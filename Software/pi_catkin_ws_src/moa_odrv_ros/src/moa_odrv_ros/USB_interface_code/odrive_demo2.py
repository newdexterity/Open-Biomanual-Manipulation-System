...
class OdrivePositionControl:
    def __init__(self):
        ...
        # print(30*"#" + " RIGHT " + 30*"#")
        # dump_errors(self.R1,True)
        # dump_errors(self.R2,True)
        ...
        rate = rospy.Rate(10)
        while (
            (self.R1.axis0.current_state != AXIS_STATE_IDLE)
            or (self.R1.axis1.current_state != AXIS_STATE_IDLE)
            or (self.R2.axis0.current_state != AXIS_STATE_IDLE)
            or (self.R2.axis1.current_state != AXIS_STATE_IDLE)
            or (self.R3.axis0.current_state != AXIS_STATE_IDLE)
            or (self.L1.axis0.current_state != AXIS_STATE_IDLE)
            ...
            check_and_print_err(self.R1, 0, "R1", "axis0")
            check_and_print_err(self.R1, 1, "R1", "axis1")
            check_and_print_err(self.R2, 0, "R2", "axis0")
            check_and_print_err(self.R2, 1, "R2", "axis1")
    ...
    def rightarmcallback(self, rArmMsg):
        ...
        try:
            self.R1.axis0.controller.input_pos = rArmMsg.data[0]
            rospy.sleep(0.1)
            self.R1.axis1.controller.input_pos = rArmMsg.data[1]
            rospy.sleep(0.1)
            self.R2.axis0.controller.input_pos = rArmMsg.data[2]
            # rospy.sleep(0.1)
            self.R2.axis1.controller.input_pos = rArmMsg.data[3]
            rospy.sleep(0.1)
            ...
            rospy.loginfo(50 * "#" + " R1 " + 50 * "#")
            dump_errors(self.R1, True)
            rospy.loginfo(50 * "#" + " R2 " + 50 * "#")
            dump_errors(self.R2, True)
    ...
    def checkerr(self):
        if self.R1.axis0.error != 0:
            print(
                ...
                % (self.R1.axis0.error, self.R1.axis0.motor.error)
            )
            self.clear_error()
            return False
            ...
            print(
                ...
                % (self.R1.axis1.error, self.R1.axis1.motor.error)
            )
            self.clear_error()
            return False
            ...
            print(
                "Failed calibration with axis1 error 0x%x, motor error 0x%x"
                % (self.R2.axis0.error, self.R2.axis0.motor.error)
            )
            self.clear_error()
            return False
        elif self.R2.axis1.error != 0:
            print(
                "Failed calibration with axis1 error 0x%x, motor error 0x%x"
                % (self.R2.axis1.error, self.R2.axis1.motor.error)
            )
            self.clear_error()
            return False
